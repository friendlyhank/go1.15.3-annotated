...
type waitq struct {
	//sudog 源src/runtime/runtime2.go
	first *sudog //第一个结点
	last  *sudog //最后一个节点
}
...
func chansend(c *hchan, ep unsafe.Pointer, block bool, callerpc uintptr) bool {
	...
	//向已经关闭的channel 发送消息会产生panic
	if c.closed != 0 {
		unlock(&c.lock)
		panic(plainError("send on closed channel"))
	}

	//第一种情况:当有 goroutine 在 recv 队列上等待时，跳过缓存队列，将消息直接发给 reciever goroutine
	//hank ?直接发送的话如何保证顺序
	...
	//第二种情况:如果channel未满,存放数据复制到缓存队列上
	//hank ?探究这情况有没有enqueue(插入节点)
	...
	//第三种情况:channel已满,加入到send队列里面,阻塞当前goroutine
 ...
 *当 send 队列不为空，分两种情况：
		 【一】缓存队列为空，直接从 send 队列的sender中接收数据 元素；
		 【二】缓存队列不为空，此时只有可能是缓存队列已满，从队列头取出元素，并唤醒 sender 将元素写入缓存队列尾部。
		 由于为环形队列，因此，队列满时只需要将队列头复制给 reciever，同时将 sender 元素复制到该位置，并移动队列头尾索引，不需要移动队列元素。
		 【这就是为什么使用环形队列的原因】
...
func chanrecv(c *hchan, ep unsafe.Pointer, block bool) (selected, received bool) {
	...
	//CASE1: channel已经关闭并且channel为空,则返回
	//这里说明了channel关闭了也可以正常的接收
...
//先进先出的队列
/**
*param sgp *sudog插入的节点
*content:
	取到最后一个节点
	设置插入节点的prev(上一个节点)为last节点
	设置last节点的next节点为插入节点
	更新last节点为插入的节点
**/
...
*content:
	取到first节点
	取到first的next节点
	将first的next节点的prev置为nil
	将first的next节点置为first
	将之前取出的first节点的next置为nil
**/
...
/*
 *问题一：channel如何保证顺序
 *问题二:针对发送和接受的三种情况,比如发送举例:
		第一种情况是，有正在接受的队列是指直接发送，发送的是数据的首地址还是当前数据？如果是首地址在哪里转换?
		第二种情况是,缓存队列没满，放入缓存队列中，这个缓存队列只是直接放在buf里，还是数据都统一放在sendq的协程里
*/
